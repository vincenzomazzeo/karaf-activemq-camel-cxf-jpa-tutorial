...
The Warehouse Service has only one entity in his data model, the `Product`. This class is a [POJO] annotated with [JPA] and [Jackson] annotations and contains two fields, `id` and `name`. The `id` field  is auto generated by the `product_sequence` created in the Warehouse DB (see TODO).  
